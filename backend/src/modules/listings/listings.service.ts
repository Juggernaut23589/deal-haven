...
function formatListingDetail(raw: any) {
  ...
  return {
    ...
    localPickup: raw.localPickup ?? false,
    shipsNationally: raw.shipsNationally ?? true,
    offerAutoAcceptThreshold: raw.autoAcceptPrice ? Number(raw.autoAcceptPrice) : null,
    offerAutoDeclineThreshold: raw.autoDeclinePrice ? Number(raw.autoDeclinePrice) : null,
    ...
    attributes: (raw.attributes ?? []).map((a: any) => ({
      ...
      slug: a.categoryAttribute?.slug ?? '',
      value: a.value,
      unit: a.categoryAttribute?.unit ?? null,
    })),
    tags: raw.tags ?? [],
    variants: raw.variants ?? [],
    allowBundleDiscount: raw.allowBundleDiscount ?? false,
    isDigital: raw.isDigital ?? false,
    digitalFileUrl: raw.digitalFileUrl ?? null,
    minOrderQuantity: raw.minOrderQuantity ?? 1,
    maxOrderQuantity: raw.maxOrderQuantity ?? null,
    acceptsReturns: raw.acceptsReturns ?? false,
    returnDays: raw.returnDays ?? null,
